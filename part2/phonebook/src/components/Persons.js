...
const deleteContact = (id, name, persons, setPersons, setFilterResult, setMessage, setErrorMessage) => {
            ...
            <h2>Numbers</h2>
            {
                props.filterResult.map((person, i) =>
                    <li key={i}> {person?.name} {person?.number}
                        <button onClick={ () => deleteContact(person?.id, person?.name, props.persons, props.setPersons, props.setFilterResult, props.setMessage, props.setErrorMessage)}> delete </button>
                    </li>
                )}
        </div>
    )
